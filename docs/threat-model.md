...
| Malicious/replaced adapter                    | Registry rejects runtime DELEGATECALL opcodes, stores runtime code hash, pair and cap, and router checks before every use                                                      | Adapter-manager compromise can still register malicious non-proxy logic; adapter source review remains mandatory                                   |
...
| RPC lies, outage, or public quote exhaustion  | Timeouts, exponential retry, fallback transports, short read caches, PostgreSQL-shared quote/prepare limits, stored block hashes and reorg rollback                            | Providers can share infrastructure; use independent vendors and edge-level abuse controls                                                          |
...
| Signer mapping tampering                      | DB constraints bind one KMS key and public signer address per stable merchant; provisioning derives the address from KMS; onboarding compares it with the canonical registry   | Database write compromise can deny service or select another attacker-controlled KMS key available to the API role; restrict DB writes and KMS IAM |
